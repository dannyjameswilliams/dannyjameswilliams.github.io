...
// [[Rcpp::export(name="cv_local_lm")]]
Rcpp::IntegerVector cv_local_lm_I(vec y, mat x0, mat X0, mat& x, mat& X, int k, mat& Hdiags)
{
  int n = x0.n_rows;
  int H_len = Hdiags.n_cols;
  
  Rcpp::IntegerVector all_ind = Rcpp::rep_each(Rcpp::seq_len(k), n/k);
  Rcpp::NumericVector mse_H(H_len);
  
  // Select different H loop
  for(int kk = 0; kk < H_len; kk++)
  {
    vec diag_for_H = Hdiags.col(kk);
    mat H_in = diagmat(diag_for_H);
    Rcpp::NumericVector mse_fit(k);
    ...
    for(int jj = 0; jj < k; jj++)
    {
      ...
      uvec int_out = (all_ind == jj);
      
      vec y_in(y.size());
      for(int yi=0; yi < int_out.size(); yi++)
      {
        y_in(yi) = y_copy(int_out(yi));
      }
      mat x_in = x_copy.rows(int_out);
      mat X_in = X_copy.rows(int_out);
      ...
      for(int ii = 0; ii < n; ii++)
      {
        ...
        fit(ii) = as_scalar(local_lm_fit_I(y, x00, X00, x, X, H_in));
        if(ii % 50 == 0) R_CheckUserInterrupt();
        
      }
      
      vec fit_cv_error = fit(int_out);
